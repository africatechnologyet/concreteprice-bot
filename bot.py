...
def calc_sale_price(grade,uc,fc,transport,margin=None):
    mat_bd,mat_cost=calc_material_cost(grade,uc);fix_bd,fix_cost=calc_fixed_cost(fc)
...
def costs_text(ctx):
    ...
    return "\n".join(lines)
(MAIN_MENU,SELECT_GRADES,ASK_VOLUME,ASK_PUMP,ASK_DISTANCE,ASK_FUEL_PRICE,SHOW_RESULT,ENTER_CUSTOM_MARGIN,SELECT_MATERIAL,ENTER_COST)=range(10)
...
async def select_grades_handler(update,context):
    ...
    if d.startswith("toggle_"):
        ...
        await q.edit_message_text("🏗  *Select Concrete Grades*\n\nTap grades to select, tap again to deselect.\nTap *Done* when ready.",parse_mode="Markdown",reply_markup=grades_kb(selected));return SELECT_GRADES
    elif d=="grades_done":
...
async def enter_custom_margin(update,context):
    ...
    grade,r=results[idx];uc=get_uc(context);fc=get_fc(context)
    new_r=calc_sale_price(grade,uc,fc,r["transport"],margin=m);results[idx]=(grade,new_r);context.user_data["results"]=results
    header=f"📊 *Result {idx+1} of {total}*\n\n" if total>1 else ""
    await update.message.reply_text(header+fmt_summary(grade,new_r),parse_mode="Markdown",reply_markup=result_kb(idx,total));return SHOW_RESULT
...
async def run():
    ...
    conv=ConversationHandler(entry_points=[CommandHandler("start",start)],states={MAIN_MENU:[CallbackQueryHandler(main_menu_handler)],SELECT_GRADES:[CallbackQueryHandler(select_grades_handler)],ASK_VOLUME:[MessageHandler(filters.TEXT&~filters.COMMAND,ask_volume),CallbackQueryHandler(ask_volume)],ASK_PUMP:[MessageHandler(filters.TEXT&~filters.COMMAND,ask_pump),CallbackQueryHandler(ask_pump)],ASK_DISTANCE:[MessageHandler(filters.TEXT&~filters.COMMAND,ask_distance),CallbackQueryHandler(ask_distance)],ASK_FUEL_PRICE:[MessageHandler(filters.TEXT&~filters.COMMAND,ask_fuel_price),CallbackQueryHandler(ask_fuel_price)],SHOW_RESULT:[CallbackQueryHandler(show_result_handler)],ENTER_CUSTOM_MARGIN:[MessageHandler(filters.TEXT&~filters.COMMAND,enter_custom_margin)],SELECT_MATERIAL:[CallbackQueryHandler(select_material_handler)],ENTER_COST:[MessageHandler(filters.TEXT&~filters.COMMAND,enter_cost),CallbackQueryHandler(enter_cost)]},fallbacks=[CommandHandler("cancel",cancel),CommandHandler("start",start)],per_message=False)
