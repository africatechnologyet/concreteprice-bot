...
FIXED_COSTS = {'Labor':[160,160,160,160,200,147,160,200,200,200,200],'Overhead':[160,160,160,160,200,147,160,200,200,200,200],'Reject 2.5%':[264,264,264,264,264,264,264,264,264,264,264],'Truck':[400,400,400,400,400,400,400,400,400,400,400],'Fuel':[317,260,200,366,200,200,260,200,200,353,244],'Pump':[400,0,0,550,234,234,600,341,366,400,705]}
...
def fmt_summary(grade,r):
    return f"*{grade} - Price Summary*\n```\nProd Cost:  ETB {r['prod_cost']:>10,.2f}\nMargin:         {r['margin']*100:.1f}%\nSale Price: ETB {r['sale_price']:>10,.2f}\nProfit:     ETB {r['profit']:>10,.2f}\n```"
def fmt_breakdown(grade,r):
    ...
    lines.append("-"*44)
    for item in FIXED_COSTS: lines.append(f"{item:<14}{'':>9}{'':>8}{bd[item]['cost']:>12,.2f}")
    lines+=["-"*44,f"{'Prod Cost':<30}{r['prod_cost']:>14,.2f}",f"{'Margin':<30}{r['margin']*100:>13.1f}%",f"{'Sale Price':<30}{r['sale_price']:>14,.2f}","```"]
    return "\n".join(lines)
def fmt_margins(grade,uc):
    ...
    lines.append("```")
    return "\n".join(lines)
(MAIN_MENU,SELECT_GRADE,GRADE_ACTION,ENTER_MARGIN,SELECT_MATERIAL,ENTER_COST)=range(6)
def mmk(): return InlineKeyboardMarkup([[InlineKeyboardButton("Get Sale Price",callback_data="menu_price")],[InlineKeyboardButton("Update Unit Costs",callback_data="menu_costs")],[InlineKeyboardButton("All Margins Table",callback_data="menu_margins")],[InlineKeyboardButton("Reset Unit Costs",callback_data="menu_reset")]])
def gk(pfx):
    rows,row=[],[]
    for g in GRADES:
...
def gak(grade): return InlineKeyboardMarkup([[InlineKeyboardButton("Price Summary",callback_data="summary_"+grade)],[InlineKeyboardButton("Full Breakdown",callback_data="breakdown_"+grade)],[InlineKeyboardButton("Custom Margin",callback_data="custom_"+grade)],[InlineKeyboardButton("Back",callback_data="menu_price"),InlineKeyboardButton("Main Menu",callback_data="goto_main")]])
def mk():
...
async def start(update,context):
    ...
    if update.message: await update.message.reply_text(text,reply_markup=mmk())
    else: await update.callback_query.edit_message_text(text,reply_markup=mmk())
    return MAIN_MENU
async def mmh(update,context):
    q=update.callback_query;await q.answer();d=q.data
    if d=="menu_price": await q.edit_message_text("Select grade:",reply_markup=gk("price"));return SELECT_GRADE
    elif d=="menu_margins": await q.edit_message_text("Select grade:",reply_markup=gk("margins"));return SELECT_GRADE
    elif d=="menu_costs":
        uc=get_uc(context);txt="Current Unit Costs (ETB)\n\n"+"\n".join(m+": "+str(c) for m,c in uc.items())+"\n\nSelect material:"
        await q.edit_message_text(txt,reply_markup=mk());return SELECT_MATERIAL
    elif d=="menu_reset": context.user_data["unit_costs"]=dict(DEFAULT_UNIT_COSTS);await q.edit_message_text("Costs reset.",reply_markup=mmk());return MAIN_MENU
    elif d=="goto_main": return await start(update,context)
async def sgh(update,context):
    q=update.callback_query;await q.answer();d=q.data
    ...
    if d.startswith("price_"):
        ...
        grade=d.split("_",1)[1]
        await q.edit_message_text(fmt_margins(grade,get_uc(context)),reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back",callback_data="menu_margins"),InlineKeyboardButton("Main Menu",callback_data="goto_main")]]));return SELECT_GRADE
...
async def gah(update,context):
    q=update.callback_query;await q.answer();d=q.data;uc=get_uc(context)
    if d=="goto_main": return await start(update,context)
    if d=="menu_price": await q.edit_message_text("Select grade:",reply_markup=gk("price"));return SELECT_GRADE
    if d.startswith("summary_"):
        grade=d.split("_",1)[1];await q.edit_message_text(fmt_summary(grade,calc_sale_price(grade,uc)),reply_markup=gak(grade));return GRADE_ACTION
    elif d.startswith("breakdown_"):
        grade=d.split("_",1)[1];await q.edit_message_text(fmt_breakdown(grade,calc_sale_price(grade,uc)),reply_markup=gak(grade));return GRADE_ACTION
    elif d.startswith("custom_"):
        grade=d.split("_",1)[1];context.user_data["grade"]=grade
        await q.edit_message_text("Enter margin % for "+grade+" (e.g. 12):");return ENTER_MARGIN
async def emh(update,context):
    grade=context.user_data.get("grade")
    try:
        m=float(update.message.text.strip().replace("%",""))/100
        if not (0<m<2): raise ValueError
    except ValueError: await update.message.reply_text("Enter a number between 1-100.");return ENTER_MARGIN
    await update.message.reply_text(fmt_summary(grade,calc_sale_price(grade,get_uc(context),m)),reply_markup=gak(grade));return GRADE_ACTION
async def smh(update,context):
    q=update.callback_query;await q.answer();d=q.data
    if d=="goto_main": return await start(update,context)
...
async def ech(update,context):
    mat=context.user_data.get("material")
    try:
        c=float(update.message.text.strip())
        if c<0: raise ValueError
    except ValueError: await update.message.reply_text("Enter a valid positive number.");return ENTER_COST
